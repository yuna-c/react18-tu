...
/*
  ...
  활용예: 비동기데이터의 pending상태가 길어질때는 대신 fallback을 통해서 정적인 UI를 대신 호출 (로딩바)


  useTransition vs Suspense의 차이
  - useTranstion은 컴포넌트간의 동기화처리가 아닌 동시에 실행되는 비동기 방식이지만 startTransition으로 묶어놓은 연산의 우선순위가 밀리는 것 뿐

  - Suspense는 해당 컴포넌트에서 관리되는 promise객체의 상태를 실시간으로 감시하면서 pending이 끝났을때 동기적으로 컴포넌트를 호출

  - Suspense를 사용하기 조건: Suspense로 동기화시키는 컴포넌트 내부에 Promise객체의 상태변화를 추적할 수 있는 로직이 구현되야함
*/
